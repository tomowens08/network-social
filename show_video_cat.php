...
<?php
     $cat_name=$videos->get_cat($HTTP_GET_VARS["cat_id"]);
     $num_cat_videos=$videos->get_cat_num_videos($HTTP_GET_VARS["cat_id"]);
?>
<b>Videos from <?=stripslashes($cat_name["cat_name"])?></b>
</td>
...
<?php
        ...
        $n_previous=$n-12;
?>
&#187; <a href='show_video_cat.php?cat_id=<?=$HTTP_GET_VARS["cat_id"]?>&n=<?=$n_previous?>'>Previous</a>
<?
        }
        if($next==1)
        {
?>
&nbsp;&#187; <a href='show_video_cat.php?cat_id=<?=$HTTP_GET_VARS["cat_id"]?>&n=<?=$n_next?>'>Next</a>
<?php
        }
?>
</td>
...
<?php
     $sr_no=0;
     $front_res=$videos->display_cat_vids($HTTP_GET_VARS["cat_id"],$n);
