...
//2 - deleted
	...
	if ($_GET['approve']) {
			$sql = "SELECT * FROM groups WHERE id = ".$_GET['group_id'];
			$res = mysql_fetch_array(mysql_query($sql));
			if ($res['member_id'] == $_SESSION['member_id']) {
				$sql = "UPDATE invitations SET status = 1 WHERE member_id = '".$_GET['member_id']."' AND group_id = '".$_GET['group_id']."'";
				mysql_query($sql);
	...
	if ($_GET['decline']) {
			...
			$res = mysql_fetch_array(mysql_query($sql));
			if ($res['member_id'] == $_SESSION['member_id']) {
				$sql = "UPDATE invitations SET status = 2 WHERE member_id = '".$_GET['member_id']."' AND group_id = '".$_GET['group_id']."'";
				mysql_query($sql);
				echo '<script>document.location.replace(\''.$_SERVER['PHP_SELF'].'\')</script>';
			}
	}
...
<?
	...
	$sql = "SELECT * FROM groups g, invitations gf WHERE g.id = gf.group_id AND g.type = 1 AND NOT gf.status AND NOT is_invited AND g.member_id = ".$_SESSION['member_id']." AND gf.date > ".$expired." ORDER BY gf.date";
	$res_g = mysql_query($sql);
?>
	Approve
	<table border="0" align="center" width="100%" cellpadding="0" cellspacing="2" bgcolor="#ffffff">
	<tr bgcolor="#6699CC">
		<td align="center"><b><font color='#FFFFFF'>Request from</font></b></td>
...
?>
	<tr bgcolor="<?=$bg_color?>" onmouseover="setPointer(this, 'over', '<?=$bg_color?>', '#DEEFFF', '');" onmouseout="setPointer(this, 'out', '<?=$bg_color?>', '#DEEFFF', '');" onmousedown="setPointer(this, 'click', '<?=$bg_color?>', '#DEEFFF', '');" class="txt_label">
		...
		<td>&nbsp;<label onclick="popUpWindow('./group_popup.php?group_id=<?=$f['group_id']?>', 200, 200, 300, 200);" style="cursor:pointer;" title="Click for details"><?=$f['group_name']?></label></td>
		<td align="center"><?=date('m/d/Y H:i:s',$f['date']);?></td>
		<td align="center"><a href="<?=$_SERVER['PHP_SELF']?>?approve=1&group_id=<?=$f['group_id']?>&member_id=<?=$mem['member_id']?>">Approve</a></td>
		<td align="center"><a href="<?=$_SERVER['PHP_SELF']?>?decline=1&group_id=<?=$f['group_id']?>&member_id=<?=$mem['member_id']?>">Decline</a></td>
	</tr>
<?
	}
...
<?
	//approve
	$i = 1;
	$sql = "SELECT * FROM groups g, invitations gf WHERE g.id = gf.group_id AND g.type = 1 AND gf.status = 1 AND NOT is_invited AND g.member_id = ".$_SESSION['member_id']." ORDER BY gf.date";
	$res_g = mysql_query($sql);
?>
	<br>
	Approved
...
<?
	//approve
	$i = 1;
	$sql = "SELECT * FROM groups g, invitations gf WHERE g.id = gf.group_id AND g.type = 1 AND gf.status = 2 AND NOT is_invited AND g.member_id = ".$_SESSION['member_id']." ORDER BY gf.date";
	$res_g = mysql_query($sql);
?>
	<br>
	Declined
